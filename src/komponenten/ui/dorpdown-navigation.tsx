      ...
      <div className="relative gap-5 flex flex-col items-center justify-center">
        <ul className="relative flex items-center space-x-0">
          {navItems.map((navItem) => (
            ...
            >
              <button
                className="text-sm py-1.5 px-4 flex cursor-pointer group transition-colors duration-300 items-center justify-center gap-1 text-muted-foreground hover:text-foreground relative animate-none"
              ...
              <AnimatePresence>
                {openMenu === navItem.label && navItem.subMenus && (
                  <div className="w-auto absolute left-0 top-full pt-2 z-50">
                    <motion.div
                      className="bg-white border border-gray-200/50 p-6 w-max shadow-xl"
                              ...
                              {sub.items.map((item) => {
                                ...
                                return (
                                  <li key={item.label}>
                                    ...
                                    >
                                      <div className="border border-gray-200 text-gray-700 rounded-md flex items-center justify-center size-9 shrink-0 group-hover:bg-[#1e5adb] group-hover:text-white transition-colors duration-300">
                                        <Icon className="h-5 w-5 flex-none" />
                                      </div>
                                      <div className="leading-5 w-max">
                                        <p className="text-sm font-bold text-gray-800 shrink-0 group-hover:text-[#1e5adb] transition-colors">
                                          {item.label}
                                        </p>
