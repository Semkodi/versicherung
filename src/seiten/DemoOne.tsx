import { DropdownNavigation } from "@/komponenten/ui/dorpdown-navigation";
import {
  Cpu,
  Globe,
  Eye,
...
export default function DemoOne() {
  return (
    <div className="relative pt-32 pb-20 gap-5 flex flex-col items-center justify-center bg-gray-50/50 min-h-screen">
      <TabsDemo />
      <NavigationDemo />
    </div>
  );
  ...
  return (
    <ul className="flex items-center justify-center">
      {ITEMS.map((item) => (
        <button
          key={item.id}
          className="py-2 relative duration-300 transition-colors hover:text-[#1e5adb] cursor-pointer"
          onClick={() => setActive(item)}
          onMouseEnter={() => setIsHover(item)}
          onMouseLeave={() => setIsHover(null)}
        >
          <div className="px-5 py-2 relative text-sm font-bold text-gray-600 hover:text-[#1e5adb]">
            {item.tile}
            {isHover?.id === item.id && (
              <motion.div
                layoutId="hover-bg-tabs"
                className="absolute inset-0 w-full h-full bg-[#1e5adb]/10 -z-10"
          ...
          {active.id === item.id && (
            <motion.div
              layoutId="active"
              className="absolute bottom-0 left-0 right-0 w-full h-0.5 bg-[#1e5adb]"
            />
          )}
          {isHover?.id === item.id && (
            <motion.div
